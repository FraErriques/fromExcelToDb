...
namespace Common.Template_Singleton
{
    ...
    ///  Template_Singleton
    ///  
    ///  NB. Non è qui presente il "vincolo-IDisposable", che obbliga il tipo generico "T", ad avere metodo pubblico "Dispose"
    ///  il costrutto e': 
    ...
    public class TSingletonNotIDispose<T> where T : new()
    {
        ...
        ///     to take advantage of GarbageCollector (gc), which executes the destructor..
        /// </summary>
        ~TSingletonNotIDispose()
        {
            TSingletonNotIDispose<T>.unsubscribe_all_();
        }// end Dtor
        ...
        {
            lock (typeof(TSingletonNotIDispose<T>))
            {
                try
                {
                    if (null == TSingletonNotIDispose<T>.handle)
                    {
                        TSingletonNotIDispose<T>.handle = new T();
                    }// else:  logDbConnection gia' istanziata
                    // in entrambi i casi iscrivo il nuovo client
                    TSingletonNotIDispose<T>.reference_counter++;
                }
            ...
            return TSingletonNotIDispose<T>.handle;
        ...
        public static void unsubscribe_one_()
        {
            lock (typeof(TSingletonNotIDispose<T>))
            {
                --TSingletonNotIDispose<T>.reference_counter;// one client less
                if (0 == TSingletonNotIDispose<T>.reference_counter)
                {
                    // kill
                    if (null != TSingletonNotIDispose<T>.handle)
                    {
                        // NB NOT in this case ! TSingletonNotIDispose<T>.handle.Dispose();
                        TSingletonNotIDispose<T>.handle = default(T);
                    }
                }
            }// end critical section
        ...
        public static void unsubscribe_all_()
        {
            lock (typeof(TSingletonNotIDispose<T>))
            {
                TSingletonNotIDispose<T>.reference_counter = 0;// unsubscribe_all_
                // kill
                if (null != TSingletonNotIDispose<T>.handle)
                {
                    // NB. not in this case : TSingletonNotIDispose<T>.handle.Dispose();
    ...
    }// end  class TSingletonNotIDispose<T>
