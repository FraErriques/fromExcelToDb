...
//			this._statistics();
        ...
        {
            string theRequiredCal = null;
            // ------- call to diverter_method----------
            bool isConfigRead =
                ConfigDiverter.Diverter.DiverterMethod(
                // DbActions.ExtractNation.ExtractionService(  NB. diverted----------!!!!!!!----for working with a .config file, instead of a db-------------
                    calendarName,
                    out theRequiredCal);
            if (!isConfigRead)
            {
