...
namespace Entity.InsertionManager
{
    ...
    /// </summary>
    public class Dbdotazioni2021_InsertionManager_Multivalent_
    {//NB. on db dotazioni2021  ################################
        ...
        private delegate int ProxyPointer(
            ...
            string matricola,
            string note,
			System.Data.SqlClient.SqlTransaction trx		// transaction
        );
        // data
        ...
        public Dbdotazioni2021_InsertionManager_Multivalent_( string inWhichTable, string fullPath )
        {// first selet which table, when there is more than one with the same record-layout.
            if (inWhichTable.Trim().ToUpper() == "PC")
            {
                theSelectdProxy = new ProxyPointer(Proxies.usp_PC_INSERT_SERVICE.usp_PC_INSERT);
            }
            else if (inWhichTable.Trim().ToUpper() == "CELLPHONE")
            {
                theSelectdProxy = new ProxyPointer(Proxies.usp_cellPhone_INSERT_SERVICE.usp_cellPhone_INSERT);
            }
            else if (inWhichTable.Trim().ToUpper() == "ACCESSORI")
            {
                theSelectdProxy = new ProxyPointer(Proxies.usp_accessori_INSERT_SERVICE.usp_accessori_INSERT);
            }
            else
            {
        ...
        /// Dtor
        /// </summary>
        ~Dbdotazioni2021_InsertionManager_Multivalent_()
        {
        ...
        {// TODO insert into originale TIM : parametrize for both.           
            ...
            {
                this.curRowSplitted = this.txtParser.rowSplitter(this.allRows[d]);// get all columns of a single row, by means of this.rowSplitter(
                Entity.Entities.Dbdotazioni2021_Entity_Multivalent_ currentRow = new Entities.Dbdotazioni2021_Entity_Multivalent_( this.curRowSplitted);
                ...
                insertionResultOnCurRow =
                    this.theSelectdProxy( //-------------------------------- either BBT xor TIM xor draft-------------------------------
                        ...
                        ,currentRow.get_oggetto()
                        ,currentRow.get_matricola()
                        , currentRow.get_note()
